...
Usage:
    In the directory of this script / executable place two files:
        -config.json like:
            {
              "FILE": "C:/Users/Pawel/Desktop/DaneTaty.csv",
              "WEATHER_DATA_PATH": "D:/weather",
              "USE_LOCAL_LANG": false
            }
        -translations.json:
            {
...
with open('config.json') as doc:
    data = json.load(doc)

FILE = data['FILE']
FILE = pathlib.Path(FILE)
WEATHER_DATA_PATH = pathlib.Path(data['WEATHER_DATA_PATH'])
WEATHER_FILENAME_FORMAT = "weather_{y}-{m:02}.json"
LOCAL_LANG = data['USE_LOCAL_LANG']

assert FILE.exists()
assert WEATHER_DATA_PATH.is_dir()
assert isinstance(LOCAL_LANG, bool)
...
with open('translations.json') as doc:
    translations = json.load(doc)
